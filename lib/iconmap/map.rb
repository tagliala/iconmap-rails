...
class Iconmap::Map
  ...
  PIN_REGEX = /^pin\s+["']([^"']+)["']/ # :nodoc:

  def self.pin_line_regexp_for(package) # :nodoc:
    /^.*pin\s+["']#{Regexp.escape(package)}["'].*$/
  end
  ...
  def pin(name, to: nil, preload: true, integrity: true)
    clear_cache
    @packages[name] = MappedFile.new(name: name, path: to || name, preload: preload, integrity: integrity)
  end
  ...
  #   packages = iconmap.preloaded_module_packages(resolver: ApplicationController.helpers)
  ...
  #   packages = iconmap.preloaded_module_packages(resolver: helpers, entry_point: "admin")
  #
  #   # Get preloaded packages for multiple entry points
  #   packages = iconmap.preloaded_module_packages(resolver: helpers, entry_point: ["application", "admin"])
  #
  #   # Use a custom cache key for different asset hosts
  #   packages = iconmap.preloaded_module_packages(resolver: helpers, cache_key: "cdn_host")
  ...
  #   end
  def digest(resolver:)
    Digest::SHA1.hexdigest(to_json(resolver: resolver))
  end
  ...
  def resolve_asset_path(path, resolver:)
    ...
    Rails.logger.warn "Iconmap skipped missing path: #{path}"
    nil
  end
  ...
  def build_integrity_hash(packages, resolver:)
    packages.filter_map do |_name, mapping|
