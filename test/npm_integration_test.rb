...
class Iconmap::NpmIntegrationTest < ActiveSupport::TestCase
  test 'successful outdated packages against live jsdelivr' do
  ...
  test 'failed outdated packages request against live bad domain' do
    ...
    original_data_uri = Iconmap::Jsdelivr.data_uri
    Iconmap::Jsdelivr.data_uri = URI('https://invalid.error')

    assert_raises(Iconmap::Jsdelivr::HTTPError) do
      npm.outdated_packages
    end
  ensure
    Iconmap::Jsdelivr.data_uri = original_data_uri
  end
